...
function normalizeVariable(value) {
  var string;
  string = value.trim().replace(/\s+/g, '_').replace(/\.+/g, '_').replace(/_+/g, '_');
...
function createIframe(src, height, allowFullScreen) {
  ...
  if (allowFullScreen) {
    if (parsedHeight > 0) {
      iframe = `<iframe src="${source}" height="${parsedHeight}" allowfullscreen></iframe>`;
    } else {
      iframe = `<iframe src="${source}" allowfullscreen></iframe>`;
    }
  } else {
    if (parsedHeight > 0) {
      iframe = `<iframe src="${source}" height="${parsedHeight}"></iframe>`;
    } else {
...
function fixPostalCode(postalCode) {
  var correctPostalCode = postalCode.trim().toUpperCase();
  var part1, part2;
  if (correctPostalCode[3] !== ' ') {
    part1 = correctPostalCode.slice(0, 3);
    part2 = correctPostalCode.slice(3, correctPostalCode.length);
    correctPostalCode = part1 + ' ' + part2;
  }

  var postalRegex = /[^DFIOQUWZ][0-9][^DFIOQU][ ][0-9][^DFIOQU][0-9]$/;

  if (postalRegex.test(correctPostalCode)) {
...
function toProvince(postalCode, useShortForm) {
  var success, returnVal;

  try {
    success = fixPostalCode(postalCode);
    var identifier = success[0];
    switch (identifier) {
      case 'K':
      case 'L':
      case 'M':
      case 'N':
      case 'P':
        useShortForm ? (returnVal = 'ON') : (returnVal = 'Ontario');
        break;
      case 'G':
      case 'H':
      case 'J':
        useShortForm ? (returnVal = 'QC') : (returnVal = 'Quebec');
        break;
      case 'B':
        useShortForm ? (returnVal = 'NS') : (returnVal = 'Nova Scotia');
        break;
      case 'E':
        useShortForm ? (returnVal = 'NB') : (returnVal = 'New Brunswick');
        break;
      case 'R':
        useShortForm ? (returnVal = 'MB') : (returnVal = 'Manitoba');
        break;
      case 'V':
        useShortForm ? (returnVal = 'BC') : (returnVal = 'British Columbia');
        break;
      case 'C':
        useShortForm ? (returnVal = 'PE') : (returnVal = 'Prince Edward Island');
        break;
      case 'S':
        useShortForm ? (returnVal = 'SK') : (returnVal = 'Saskatchewan');
        break;
      case 'T':
        useShortForm ? (returnVal = 'AB') : (returnVal = 'Alberta');
        break;
      case 'A':
        useShortForm ? (returnVal = 'NL') : (returnVal = 'Newfoundland and Labrador');
        break;
      case 'X':
        useShortForm ? (returnVal = 'NT') : (returnVal = 'Northwest Territories and Nunavut');
        break;
      case 'Y':
        useShortForm ? (returnVal = 'YT') : (returnVal = 'Yukon');
        break;
      default:
        returnVal = null;
        break;
...
function normalizeCoord(value) {
  var lat, lng, latLngVal;
  var checkValue = /[+-]?[0-9]+\.?[0-9]+/g;

  latLngVal = value.match(checkValue);
...
function formatCoords(...values) {
  ...
  for (i = 0; i < values.length; i++) {
    try {
      if (i === 0) formattedCoords = normalizeCoord(values[i]);
      else formattedCoords = formattedCoords + ', ' + normalizeCoord(values[i]);
    } catch (err) {
      continue;
    }
  }
  if (typeof formattedCoords === 'undefined') {
    formattedCoords = '';
  }
...
function countForProvince(provinceCode, ...postalCodes) {
  ...
  postalCodes.forEach((element) => {
    ...
    if (provinceCode === toProvince(element, true)) {
      count++;
    }
  });
  return count;
...
function generateLicenseLink(licenseCode, targetBlank) {
  ...
  switch (licenseCode) {
    case 'CC-BY':
      ...
      break;
    case 'CC-BY-NC':
      description = 'Creative Commons Attribution-NonCommercial License';
      break;
    case 'CC-BY-SA':
      description = 'Creative Commons Attribution-ShareAlike License';
      break;
    case 'CC-BY-ND':
      description = 'Creative Commons Attribution-NoDerivs License';
      break;
    case 'CC-BY-NC-SA':
      description = 'Creative Commons Attribution-NonCommercial-ShareAlike License';
      break;
    case 'CC-BY-NC-ND':
      description = 'Creative Commons Attribution-NonCommercial-NoDerivs License';
      break;

    default:
      validLink = `https://choosealicense.com/no-permission/`;
      description = 'All Rights Reserved';
      break;
  }
  if (validLink === '') {
    linkPart = licenseCode.substring(3).toLowerCase();
    validLink = `https://creativecommons.org/licenses/${linkPart}/4.0/`;
  }

  if (targetBlank) {
    correctHTMLTag = `<a href="${validLink}" target="_blank">${description}</a>`;
  } else {
    correctHTMLTag = `<a href="${validLink}">${description}</a>`;
  }

  return correctHTMLTag;
}
...
function pureBool(value) {
  ...
  if (typeof value === 'number') {
    value > 0 ? (returnVal = true) : (returnVal = false);
  } else if (typeof value === 'boolean') {
    value ? (returnVal = true) : (returnVal = false);
  } else if (typeof value === 'string') {
    var truthyValues = ['YES', 'Y', 'OUI', 'O', 'TRUE', 'T', 'VRAI', 'V'];

    truthyValues.includes(value.toUpperCase()) ? (returnVal = true) : (returnVal = false);
  } else {
    throw 'invalid value';
  }
...
function buildUrl(query, order, count, license) {
  ...
  if (!expectedLicense.includes(license)) {
    throw 'Invalid Data';
  }
  finalURL = `https://api.inaturalist.org/v2/observations?query=${encodeURIComponent(
    query
  )}&count=${count}&order=${order}&license=${license}`;
